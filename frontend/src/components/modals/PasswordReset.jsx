...
function PasswordRecovery() {
  ...
  return (
    <div className="absolute top-0 left-0 w-full z-50 bg-black/50 backdrop-blur-[4px] px-4 py-10 flex justify-center pt-24">
      <div className="relative bg-[#FCFCF7] w-full max-w-lg min-h-screen rounded-3xl shadow-xl flex flex-col items-center border border-[#CBCBCB] px-6 pb-10 pt-24">
        {/* Logo flotante */}
        <div className="absolute -top-12 left-1/2 transform -translate-x-1/2">
          <img
            src="src/assets/logo.png"
            alt="Logo Patas Pirque"
            className="w-30 h-30 rounded-full object-containt drop-shadow-lg"
          />
        </div>

        <h2 className="text-2xl font-bold mb-2 mt-3 text-center font-secundary text-[#393939]">
          Recuperar contraseña
        </h2>
        <p className="text-base px-10 font-normal text-[#767575] mb-6 text-center">
        Ingresa tu correo electrónico para restablecer tu contraseña.
        </p>

        <div className="mb-6 w-full">
          <label className="block text-base font-semibold pl-12 text-left text-[#595146] mb-2">
          ...
          <div className="flex justify-center gap-4">
            {code.map((valor, i) => (
              <input
                ...
                className="w-12 h-14 border-1 border-[#76757580] rounded-xl text-center text-lg focus:outline-none focus:border-1 focus:border-primary"
              />
            ))}
          </div>
          <p className="text-base font-normal text-[#A4A4A4] text-center mt-3">
        ...
        <form onSubmit={handleSubmit} className="w-full space-y-5">
          <div>
            <label className="block text-base pl-12 font-semibold text-[#595146] mb-2">
              Nueva contraseña
            </label>
            <div className="relative">
              <input
                ...
                className="w-96 ml-10 px-4 py-2 bg-white border border-primary rounded-full text-sm focus:outline-none focus:border-2 focus:border-primary"
              />
              <span
                onClick={() => setShowPassword((prev) => !prev)}
                className="absolute inset-y-0 right-10 flex items-center text-primary cursor-pointer">
                {showPassword ? (
                  <svg
                    width="20"
                    height="16"
          ...
          <div>
            <label className="block mt-10 text-base pl-12 font-semibold text-[#595146] mb-2">
            ...
            <div className="relative">
              ...
              <span
                onClick={() => setShowPassword((prev) => !prev)}
                className="absolute inset-y-0 right-10 pr-3 flex items-center text-primary cursor-pointer">
                {showPassword ? (
                  <svg
                    width="20"
                    height="16"
              ...
              {errors.confirmPassword && (
                <p className="text-red-500 ml-12 text-sm mt-1">
                  {errors.confirmPassword}
                </p>
              )}
            </div>
          ...
          <div className="flex flex-col gap-4 items-center mt-6">
            <button
              type="submit"
              className="w-auto px-7 mt-10 text-lg py-2 bg-primary text-white font-semibold rounded-full shadow-lg shadow-[#00000040]">
              Restablecer contraseña
            </button>
            <button
              type="button"
              className="w-auto px-7 mt-1 text-lg py-1 border-2 border-primary text-primary font-semibold rounded-full shadow-sm shadow-[#00000040]">
              Volver atrás
            </button>
          </div>
        </form>
