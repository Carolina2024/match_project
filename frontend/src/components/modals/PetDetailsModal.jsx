      ...
      <div className="bg-white rounded-xl p-6 max-w-4xl w-full relative shadow-lg">
        ...
        <div className="flex gap-4 mb-4">
          {pet?.photos?.map((url, i) => (
            <img key={i} src={url} alt="Mascota" className="w-24 h-24 rounded object-cover" />
          ))}
        </div>

        {/* Historia */}
        <div className="mb-4">
          <h3 className="font-semibold mb-1 text-gray-700">Historia de la mascota</h3>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-x-6 gap-y-4 mt-4">
          <div className="flex flex-col">
            <span className="text-ls text-black-500 font-semibold">Mascota</span>
            <span className="text-sm text-gray-800">{pet?.name}</span>
          </div>
          <div className="flex flex-col">
            <span className="text-ls text-black-500 font-semibold">Especie</span>
          ...
          <div className="flex flex-col">
            <span className="text-ls text-black-500 font-semibold">Actividad</span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-6">
          ...
          <div>
            <h3 className="font-semibold text-gray-700 mb-1">Rasgos de la mascota</h3>
            <div className="flex flex-wrap gap-2">
              {pet?.traits?.map((trait, i) => (
                <span key={i} className="bg-orange-200 text-orange-800 px-3 py-1 rounded-full text-xs">
                  {trait}
                </span>
              ))}
            </div>
