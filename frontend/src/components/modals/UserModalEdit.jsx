    ...
    const fetchUser = async () => {
      try {
        ...
        const userData = {
          fullname: res.fullname,
          email: res.email,
          password: "", // no se debe mostrar hash en el form
          birthDate: res.adopter.birthDate,
          phoneNumber: res.adopter.phoneNumber,
          identityDocument: res.adopter.identityDocument,
          address: res.adopter.address,
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      <Controller
        ...
        control={control}
        render={({ field }) => (
          <RadioGroup label="¿Permiten mascotas?" {...field} />
        )}
      />

      {/* Otros Controllers para los campos personalizados */}
...
const RadioGroup = ({ value, onChange, label }) => {
  const options = [
    { label: "SI", val: true },
    { label: "NO", val: false },
  ];
